    ...
    <footer className="relative w-full bg-[#020408] text-white overflow-hidden pt-24 pb-12 border-t border-white/5">
      ...
      <div className="relative z-10 container mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 mb-24">
          ...
          <div className="flex flex-col justify-center">
            <h2 className="text-4xl sm:text-5xl font-medium tracking-tight mb-6 leading-[1.1]">
              {t.footer.ctaTitlePart1} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">
                {t.footer.ctaTitlePart2}
              </span>
            </h2>
              ...
              <form className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <label htmlFor="name" className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{t.footer.form.nameLabel}</label>
                    <input 
                      type="text" 
                      id="name"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="email" className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{t.footer.form.emailLabel}</label>
                    <input 
                      type="email" 
                      id="email"
                ...
                <div className="space-y-2">
                    <label htmlFor="message" className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{t.footer.form.messageLabel}</label>
                    <textarea 
                      id="message"
                      rows={3}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10 mb-20">
          ...
          <div>
            <h4 className="font-semibold text-white mb-6">{t.footer.columns.company}</h4>
            <ul className="space-y-3 text-sm text-slate-400 font-light">
              {[t.footer.links.about, t.footer.links.cases, t.footer.links.careers, t.footer.links.blog].map(link => (
          ...
          <div>
            <h4 className="font-semibold text-white mb-6">{t.footer.columns.solutions}</h4>
            <ul className="space-y-3 text-sm text-slate-400 font-light">
              {[t.footer.links.consulting, t.footer.links.acceleration, t.footer.links.incubation, t.footer.links.funds].map(link => (
                <li key={link}><Link href="#" className="hover:text-blue-400 transition-colors flex items-center gap-1 group"><span className="w-0 overflow-hidden group-hover:w-2 transition-all duration-300">-</span>{link}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
             <h4 className="font-semibold text-white mb-6">{t.footer.columns.legal}</h4>
            <ul className="space-y-3 text-sm text-slate-400 font-light">
              {[t.footer.links.privacy, t.footer.links.terms, t.footer.links.cookies].map(link => (
                <li key={link}><Link href="#" className="hover:text-blue-400 transition-colors flex items-center gap-1 group"><span className="w-0 overflow-hidden group-hover:w-2 transition-all duration-300">-</span>{link}</Link></li>
              ))}
            </ul>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-white/5 pb-8 md:pb-0">
          ...
          <p className="text-xs text-slate-600 font-medium order-1 md:order-2 md:text-right md:pr-24">
            &copy; {new Date().getFullYear()} HockeyStick MX. {t.footer.copyright}
          </p>
